...
def us_market():
    import akshare as ak
    res = []
    for s,n in [(".DJI","道指"),(".IXIC","纳指"),(".INX","标普500")]:
        try:
...
def all_news():
    ...
    try:
        ...
        if df is not None and not df.empty:
            ...
            title_col = "新闻标题" if "新闻标题" in cols else cols[0]
...
def build_html(ash, usm, oil, ac, news, me_news, bt):
    ...
    ach = ""
    if ac:
        sgn = "🔴" if ac['p']>0 else ("🟢" if ac['p']<0 else "")
        ach = f"<p><b>✈️ 国航601111:</b> {ac['c']}元 {sgn} {c(ac['p'])}</p>"

    oor = "".join(f"<tr><td>{i['n']}</td><td>${i['pr']}</td><td>{c(i['p'])}</td></tr>" for i in oil)
    oh = f"<h3>🛢️ 原油（油价↗=利空航空）</h3><table border=1 cellpadding=4>{oor}</table>" if oor else ""
...
def main():
    ...
    ac = air_china(); print(f"  国航: {'OK' if ac else '重试中...'}")
